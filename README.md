...
| `GET` | `/api/plans/{id}/chart.svg` | The same data, already rendered as a self-contained SVG image (no JS, no charting library): three stacked panels (price, energy flows, battery SOC). |
| `POST` | `/api/plans/{id}/export/home-assistant` | Push a computed plan to a Home Assistant webhook. See [`docs/home-assistant-integration.md`](docs/home-assistant-integration.md). |
